      ...
      <div class="card-body">
        ...
        <div class="tab-content">
          <div id="home" class="container tab-pane active"><br>
              <canvas id="luassawah" width="800px" height="400px"></canvas>
          </div>
          <div id="menu1" class="container tab-pane fade"><br>
            <table class="table table-striped">
              <caption style="caption-side:top;">Luas Areal Persawahan</caption>
              <tr>
                <thead>
                  <th>No.</th>
                  <th>Tahun</th>
                  <th style="text-align: center">Jumlah Petak</th>
              ...
              <?php
                ...
                foreach ($dataMaster as $dataM) {
                    ?>
                <form action="/psda/luassawah/destroy/{{$dataM->id}}" id="form{{$dataM->id}}" method="post">
                  <tr>
                    <input type="hidden" name="luassawahMaster{{$dataM->id}}" value="">
                    <td>{{$nomor}}</td>
                    <td>{{$dataM->ta}}</td>
                    <td style="text-align: center">{{$dataM->totalPetak}}</td>
                    ...
                    <td>
                      ...
                      <a href="/psda/luassawah/edit/{{$dataM->id}}"><img src="/image/edit.png" alt=""></a>
 ...
 ?>
    ...
    <div class="collapse show" id="collapseExample">
      <div class="card">
        ...
        <div class="card-body">
          <form class="form-group" action="/psda/luassawah/update/{{$kodeEdit}}" method="post">
                ...
                <?php
                  ...
                  foreach ($dataDetail as $dataD) {
                    ...
                    <tr>
                      ...
                      <td>
                        <input type="number" id="jmlPetak{{$nomor}}" class="form-control" name="jmlPetak{{$nomor}}" value="{{$dataD->jmlPetak}}">
                      </td>
                      <td>
                        <input type="number" id="jmlLuas{{$nomor}}" class="form-control" name="jmlLuas{{$nomor}}" value="{{$dataD->jmlLuas}}">
                      </td>
                    </tr>
                <?php
                  $nomor ++;
                 ...
                 <tr>
                   ...
                   <td><b>Total</b></td>
                   <td>
                     <input type="number" id="totalPetak" class="form-control" name="totalPetak" value="{{$dataE->totalPetak}}" readonly>
                   </td>
                   <td>
                     <input type="number" id="totalLuas" class="form-control" name="totalLuas" value="{{$dataE->totalLuas}}" readonly>
                   </td>
                 </tr>
                 {{csrf_field()}}
              </table>
  ...
  function showUser(str) {
      if (str == "") {
          ...
          xmlhttp.open("GET","{{ url('psda/luassawah/detail') }}"+'/'+str,true);
          xmlhttp.send();
      }
  }
  </script>
   ...
   ?>
    $("#jmlLuas{{$nopu}}").keyup(function() {
      ...
      <?php
        $nojk = 1;
        foreach ($uptd as $pk){ ?>
          ksd = ksd + Number($("#jmlLuas{{$nojk}}").val());
      <?php
        $nojk ++;
      } ?>
        $("#totalLuas").val(ksd);
      ...
      $("#jmlPetak{{$nopu}}").keyup(function() {
        ksd = 0;
        <?php
          $nojk = 1;
          foreach ($uptd as $pk){ ?>
            ksd = ksd + Number($("#jmlPetak{{$nojk}}").val());
        <?php
          $nojk ++;
        } ?>
          $("#totalPetak").val(ksd);
...
<script>
  var ctx = document.getElementById("luassawah");
